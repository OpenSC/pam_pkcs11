...
static int verify_crl(X509_CRL * crl, X509_STORE_CTX * ctx) {
	...
	EVP_PKEY_free(pkey);
	if (rv < 0) {
		set_error("X509_CRL_verify() failed: %s",
...
static int check_for_revocation(X509 * x509, X509_STORE_CTX * ctx,
	...
	if (policy == CRLP_NONE) {
		...
		rv = X509_STORE_get_by_subject(ctx, X509_LU_CRL,
				X509_get_issuer_name(x509), &obj);
		if (rv <= 0) {
		...
		dist_points = X509_get_ext_d2i(x509, NID_crl_distribution_points, NULL,
				NULL);
		if (dist_points == NULL) {
			/* if there is not crl distribution point in the certificate hava a look at the ca certificate */
			rv = X509_STORE_get_by_subject(ctx, X509_LU_X509,
					X509_get_issuer_name(x509), &obj);
			if (rv <= 0) {
				set_error("no dedicated ca certificate available");
				return -1;
			}
			dist_points = X509_get_ext_d2i(obj.data.x509,
					NID_crl_distribution_points, NULL, NULL);
			X509_OBJECT_free_contents(&obj);
			if (dist_points == NULL) {
				set_error(
						"neither the user nor the ca certificate does contain a crl distribution point");
				return -1;
			}
		}
		crl = NULL;
		for (i = 0; i < sk_DIST_POINT_num(dist_points) && crl == NULL; i++) {
			...
			if (point->distpoint != NULL
					&& GET_FULLNAME(point->distpoint) != NULL) {
				for (j = 0;
						j < sk_GENERAL_NAME_num(GET_FULLNAME(point->distpoint));
						j++) {
					name =
							sk_GENERAL_NAME_value(GET_FULLNAME(point->distpoint), j);
...
static int add_file(X509_LOOKUP *lookup, const char *file) {
	...
	if (rv != 1) {
		set_error("X509_LOOKUP_load_file(ASN1) failed: %s",
				ERR_error_string(ERR_get_error(), NULL));
...
static X509_STORE * setup_store(cert_policy *policy) {
	...
	if (store == NULL) {
		set_error("X509_STORE_new() failed: %s",
				ERR_error_string(ERR_get_error(), NULL));
		return NULL;
	}
	...
	if ((is_dir(policy->ca_dir) > 0) || (is_dir(policy->crl_dir) > 0)) {
		...
		if (!lookup) {
			...
			set_error("X509_STORE_add_lookup(hash_dir) failed: %s",
					ERR_error_string(ERR_get_error(), NULL));
			return NULL;
		}
	}
	...
	if ((is_file(policy->ca_dir) > 0) || (is_file(policy->crl_dir) > 0)) {
		...
		if (!lookup) {
			X509_STORE_free(store);
			set_error("X509_STORE_add_lookup(file) failed: %s",
					ERR_error_string(ERR_get_error(), NULL));
			return NULL;
		}
	}
	...
	add_store_error: DBG1("setup_store() error: '%s'", get_error());
...
int verify_certificate(X509 * x509, cert_policy *policy) {
	...
	if (store == NULL) {
		set_error("setup_store() failed: %s",
				ERR_error_string(ERR_get_error(), NULL));
		return -1;
	}
	...
	if (ctx == NULL) {
		X509_STORE_free(store);
		set_error("X509_STORE_CTX_new() failed: %s",
				ERR_error_string(ERR_get_error(), NULL));
		return -1;
	}
...
int verify_signature(X509 * x509, unsigned char *data, int data_length,
	...
	if (pubkey == NULL) {
		set_error("X509_get_pubkey() failed: %s",
				ERR_error_string(ERR_get_error(), NULL));
		return -1;
	}
	...
	if (rv != 1) {
		set_error("EVP_VerifyFinal() failed: %s",
				ERR_error_string(ERR_get_error(), NULL));
		return -1;
	}
