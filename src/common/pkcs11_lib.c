 ...
 *	                ^maximum length
 */
static int memcmp_pad_max(void *d1, size_t d1_len, void *d2, size_t d2_len,
		size_t max_sz) {
...
int load_pkcs11_module(const char *module, pkcs11_handle_t **hp) {
	...
	DBG3("module permissions: uid = %d, gid = %d, mode = %o", module_stat.st_uid, module_stat.st_gid, module_stat.st_mode
			& 0777);
	...
	C_GetFunctionList_ptr = (CK_C_GetFunctionList) dlsym(h->module_handle,
			"C_GetFunctionList");
...
static int refresh_slots(pkcs11_handle_t *h) {
	CK_ULONG i;
	int j;
...
int init_pkcs11_module(pkcs11_handle_t *h, int flag) {
	 ...
	 */
	CK_C_INITIALIZE_ARGS initArgs = { .CreateMutex = NULL, .DestroyMutex = NULL,
			.LockMutex = NULL, .UnlockMutex = NULL, .flags = CKF_OS_LOCKING_OK,
			.pReserved = NULL };
...
int find_slot_by_number(pkcs11_handle_t *h, unsigned int slot_num,
		unsigned int *slot) {
	/* zero means find the best slot */
	if (slot_num == 0) {
		for (slot_num = 0;
				slot_num < h->slot_count && !h->slots[slot_num].token_present;
				slot_num++)
			;
	} else {
...
int find_slot_by_number_and_label(pkcs11_handle_t *h, int wanted_slot_id,
		const char *wanted_token_label, unsigned int *slot_num)
{
	int rv;
 ...
 * -1.
 */
int find_slot_by_slotlabel(pkcs11_handle_t *h, const char *wanted_slot_label,
		unsigned int *slot_num) {
	unsigned long idx;
	size_t len;
...
int find_slot_by_slotlabel_and_tokenlabel(pkcs11_handle_t *h,
		const char *wanted_slot_label, const char *wanted_token_label,
		unsigned int *slot_num) {
	unsigned long i;
	int rv;
	...
	if (strcmp(wanted_slot_label, "none") == 0) {
		for (i = 0; i < h->slot_count; i++) {
			if (h->slots[i].token_present
					&& strcmp(wanted_token_label, (char *) h->slots[i].label)
							== 0) {
				*slot_num = i;
				return (0);
			}
		...
		for (i = 0; i < h->slot_count; i++) {
			if (h->slots[i].token_present) {
				...
				if ((memcmp_pad_max((void *) slot_label, strlen(slot_label),
						(void *) wanted_slot_label, strlen(wanted_slot_label),
						64) == 0)
						&& (memcmp_pad_max((void *) token_label,
								strlen(token_label),
								(void *) wanted_token_label,
...
int wait_for_token(pkcs11_handle_t *h, int wanted_slot_id,
		const char *wanted_token_label, unsigned int *slot_num) {
	int rv;

	do {
		/* see if the card we're looking for is inserted */
		rv = find_slot_by_number_and_label(h, wanted_slot_id,
				wanted_token_label, slot_num);
		if (rv != 0) {
			/* could call C_WaitForSlotEvent, for now just poll */
			sleep(10);
...
int open_pkcs11_session(pkcs11_handle_t *h, unsigned int slot) {
	...
	rv = h->fl->C_OpenSession(h->slots[slot].id, CKF_SERIAL_SESSION, NULL, NULL,
			&h->session);
	if (rv != CKR_OK) {
...
cert_object_t **get_certificate_list(pkcs11_handle_t *h, int *ncerts) {
	...
	CK_ATTRIBUTE cert_template[] = { { CKA_CLASS, &cert_class,
			sizeof(CK_OBJECT_CLASS) }, { CKA_CERTIFICATE_TYPE, &cert_type,
			sizeof(CK_CERTIFICATE_TYPE) }, { CKA_ID, NULL, 0 }, { CKA_VALUE,
			NULL, 0 } };
	...
	while (1) {
		...
		if (rv != CKR_OK) {
			set_error("CertID length: C_GetAttributeValue() failed: 0x%08lX",
					rv);
			goto getlist_error;
		}
		...
		if (id_value == NULL ) {
			set_error("CertID malloc(%d): not enough free memory available",
		...
		if (rv != CKR_OK) {
			free(id_value);
			set_error("CertID value: C_GetAttributeValue() failed: 0x%08lX",
					rv);
			goto getlist_error;
		}
		...
		if (cert_value == NULL ) {
			set_error(
					"Cert Length malloc(%d): not enough free memory available",
		...
		x509 = d2i_X509(NULL, (const unsigned char **) &cert_template[3].pValue,
				cert_template[3].ulValueLen);
		if (x509 == NULL ) {
			free(id_value);
			free(cert_value);
			set_error("d2i_x509() failed: %s",
					ERR_error_string(ERR_get_error(), NULL ));
			goto getlist_error;
		}
		/* finally add certificate to chain */
		certs = realloc(h->certs,
				(h->cert_count + 1) * sizeof(cert_object_t *));
		if (!certs) {
			free(id_value);
			X509_free(x509);
		...
		h->certs[h->cert_count] = (cert_object_t *) calloc(
				sizeof(cert_object_t), 1);
	...
	*ncerts = h->cert_count;
	...
	getlist_error: rv = h->fl->C_FindObjectsFinal(h->session);
	if (rv != CKR_OK) {
		set_error("C_FindObjectsFinal() failed: 0x%08lX", rv);
	}
	free_certs(h->certs, h->cert_count);
...
int get_private_key(pkcs11_handle_t *h, cert_object_t *cert) {
	...
	CK_ATTRIBUTE key_template[] = {
			{ CKA_CLASS, &key_class, sizeof(key_class) }, { CKA_SIGN, &key_sign,
					sizeof(key_sign) }, { CKA_ID, NULL, 0 } };
	CK_OBJECT_HANDLE object;
	CK_ULONG object_count;
	int rv;
	...
	get_privkey_failed: rv = h->fl->C_FindObjectsFinal(h->session);
	if (rv != CKR_OK) {
		set_error("C_FindObjectsFinal() failed: 0x%08lX", rv);
	}
	return -1;
...
int sign_value(pkcs11_handle_t *h, cert_object_t *cert, CK_BYTE *data,
	...
	DBG5("hash[%ld] = [...:%02x:%02x:%02x:...:%02x]", sizeof(hash), hash[15], hash[16], hash[17], hash[sizeof(hash)
			- 1]);
	/* sign the token */
	...
	*signature_length = 128;
	while (*signature == NULL ) {
		*signature = malloc(*signature_length);
		...
		rv = h->fl->C_Sign(h->session, hash, sizeof(hash), *signature,
				signature_length);
	...
	DBG5("signature[%ld] = [%02x:%02x:%02x:...:%02x]", *signature_length, (*signature)[0], (*signature)[1], (*signature)[2], (*signature)[*signature_length
			- 1]);
	return 0;
}
...
int get_random_value(unsigned char *data, int length) {
	...
	close(fh);
	DBG5("random-value[%d] = [%02x:%02x:%02x:...:%02x]", length, data[0], data[1], data[2], data[length
			- 1]);
	return 0;
}
#endif /* HAVE_NSS */
